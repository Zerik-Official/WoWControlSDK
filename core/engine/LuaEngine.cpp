...
typedef int         (__cdecl* lua_ProtectedCall_t)  (lua_State* L, int stackIdx);
...
static lua_ProtectedCall_t  s_protectedCall = nullptr;
...
bool initialize()
{
    ...
    s_protectedCall = reinterpret_cast<lua_ProtectedCall_t> (Offsets::Lua::LUA_PROTECTED_CALL);
    ...
    if (!s_execute || !s_loadbuffer || !s_gettop || !s_settop || !s_tolstring || !s_protectedCall)
        return false;
...
{
    lua_State* L = getLuaState();
    if (!L || !s_protectedCall || !s_loadbuffer || !s_gettop || !s_settop || !s_tolstring) {
    ...
    int nresults = s_protectedCall(L, topBefore + 1);
    if (nresults < 0) {
        s_settop(L, topBefore);
        return {false, "pcall error", {}};
    }

    int currentTop = s_gettop(L);
    int startIdx = currentTop - nresults + 1;
    std::vector<LuaValue> vals;
    vals.reserve(nresults);

    for (int i = startIdx; i <= currentTop; ++i) {
        vals.push_back(readStackValue(L, i, s_type, s_toboolean, s_isnumber, s_tonumber, s_tolstring));
    }
